...
const TransactionsView: React.FC<TransactionsViewProps> = ({
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter((t) => {
      ...
      const accountTypeMatch =
        accountTypeFilter === "all" || t.accountType === accountTypeFilter;
      const categoryMatch =
  ...
  const getStatusChip = (status: "Pago" | "Pendente") => {
    ...
    if (status === "Pago") {
      return (
        <span
          className={`${baseClasses} bg-success/20 text-success dark:text-success-light`}
        >
          Pago
        </span>
      );
    ...
    return (
      <span
        className={`${baseClasses} bg-warning/20 text-yellow-800 dark:text-warning`}
      >
        Pendente
      </span>
    );
  ...
  const selectStyles =
    "block w-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg shadow-sm py-2.5 px-3 text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-primary/80 focus:border-primary transition-colors";
            ...
            <tbody>
              {paginatedTransactions.map((t) => (
                ...
                >
                  ...
                  <td
                    className={`px-6 py-4 font-semibold text-danger dark:text-danger-light`}
                  >
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className="text-primary hover:text-primary-hover dark:text-primary-light mr-4"
                    >
                      <PencilIcon />
                    </button>
                    <button
                      onClick={() => handleDelete(t.id)}
                      className="text-danger hover:text-danger-hover dark:text-danger-light"
                    >
                      <TrashIcon />
                    </button>
                  </td>
          ...
          <div className="flex justify-between items-center p-4 border-t dark:border-slate-800">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 text-sm font-medium text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Anterior
              </button>
              <button
