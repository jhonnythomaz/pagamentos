...
const ReportsView: React.FC<ReportsViewProps> = ({ transactions }) => {
  ...
  const monthlyData = useMemo(() => {
    const data: {
      [key: string]: { name: string; despesa: number; sortKey: string };
    } = {};
    filteredTransactions.forEach((t) => {
      ...
      const sortKey = `${date.getUTCFullYear()}-${String(
        date.getUTCMonth() + 1
      ).padStart(2, "0")}`;
  ...
  const handleExportCSV = () => {
    ...
    monthlyData.forEach((row) => {
      const formattedDespesa = row.despesa.toLocaleString("pt-BR", {
        style: "decimal",
    ...
    expenseByCategoryData.forEach((row) => {
      const formattedValue = row.value.toLocaleString("pt-BR", {
        style: "decimal",
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });
